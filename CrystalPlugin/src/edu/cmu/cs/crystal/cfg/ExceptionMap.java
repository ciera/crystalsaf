...
/**
 * This class maintains the exception information for the CFG algorithm
 * while it is building the graph.
 * It is notified of certain points in the cfg algorithm (such as finding a
 * finally or catch), and it can be queried for information about the finallys
 * at a given point or the nearest catch.
 * 
 * The ExceptionMap maintains links to ASTNodes and CFGNodes. It should be deleted
 * when the CFG is done with it.
 * 
 * @author cchristo
 *
 * @param <Node>
...
public class ExceptionMap<Node extends ICFGNode> implements Cloneable {
	...
	/**
	 * Notify the exception map that we've found a try node,
	 * and we have a finally for it. Finallys should be pushed
	 * before their corresponding catches. If a try has no finally,
	 * it should push on null.
	 * @param finallyNode
	 */
	...
	/**
	 * Notify the exception map that we have an exception, and it is
	 * catching a particular type of exception. Catches should be
	 * pushed in the opposite order (bottom first).
	 * @param catchNode
	 * @param exception
	 */
	 ...
	 * @return the catch node that will catch this exception, or null
	 ...
	 * This will not include the finally node at the exception catch. The returned
	 * stack will be that the nearest finally is at the top of the stack. If the
	 * exception is null, then this returns the entire stack of nodes.
	 * @param exceptionToStopAt The exception we should stop at. This might be a subtype
	 * of the actual exception that we stop at.
	...
	public Stack<Node> getFinallyToException(ITypeBinding exceptionToStopAt) {
		...
		Node finallyToStopAt, currentFinally = null, lastFinally = null;
		...
		for (int ndx = catchStack.size() - 1; ndx >= 0; ndx--) {
			...
			else if (!block.finallyNode.equals(currentFinally)) {
				lastFinally = currentFinally;
				currentFinally = block.finallyNode;
			}	
		}
		...
		int ndx = finallyStack.search(lastFinally);
